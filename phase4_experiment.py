...
def run_phase4_experiment(gamma_values, gamma2_values):
    """
    ...
    Examines how reversing the lanes on the northbound exit affects the performance of the PM networks.

    Runs the networks using the optimized parameters, and creates animation and pictures.
    
    Stores results to phase4_experiments/.
    """
    ...
    for i, gamma_val in enumerate(gamma_values):
        nt_opt_val = 10  # Fixed nt_opt value for phase 4
        ...
        network_pm_base, network_pm_2, network_pm_3, network_pm_4, network_pm_5 = setup_pm_networks(gamma_val, gamma2_val, phase4=True)
        
        # Step 3: Create output directory for this gamma
        gamma_str = f"{gamma_val:.4f}"
        gamma2_str = f"{gamma2_val:.4f}"
        output_dir = os.path.join("phase4_experiments", f"gamma_{gamma_str}_gamma2_{gamma2_str}", f"2_lanes")
        ...
        networks_strings = ("AM Base", "AM 2", "AM 3",
                    "PM Base", "PM 2", "PM 3", "PM 4")
        nts = (87000, 33600, 20400, 18600, 37800, 4200, 6600, 13800)
        # nts = (870, 336, 204, 186, 378, 42, 66, 138)
        try:
            prev = None
            for i, network in enumerate(networks):
                ...
                base_script.run_network(prev, network,
                                          nts[i], pref_file,
                                          output_dir, create_animation=new_network, step=100, create_picture=new_network, tag=f"_{networks_strings[i]}")
                prev = network
...
gamma_values = [0.0375, 0.0375, 0.0375, 0.0375, 0.0375]
gamma2_values = [0.0375, 0.1000]
if __name__ == "__main__":
    print(f"Gamma values to test: {gamma_values}")
    print(f"Gamma2 values to test: {gamma2_values}")
    run_phase4_experiment(gamma_values, gamma2_values)
    print("\nSweep completed!")