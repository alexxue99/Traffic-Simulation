...
def average_phase2_results(gamma_val, nt_opt_val, num_trials):
    """
    Averages results from multiple trials for a given gamma and nt_opt_val.
    """
    for i, gamma_val in enumerate(gamma_values):
        nt_opt_val = nt_opt_values[i]
        print(f"\nExperiment {i+1}/{len(gamma_values)}: gamma = {gamma_val:.4f}")
...
if __name__ == "__main__":
    ...
    print("\nSweep completed!")
    average_phase2_results(gamma_values[0], nt_opt_values[0], num_trials=10)
